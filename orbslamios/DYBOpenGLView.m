...
{
    float imageRatio = 4/3.0;
    float screenRatio = self.bounds.size.height/(float)self.bounds.size.width;
    _ratio = imageRatio / screenRatio;
    ...
    if (pixelBuffer != NULL) {
        ...
        int offset = _backingHeight * (1.0-_ratio) / 2;
        glViewport(0, offset, _backingWidth, _backingHeight-2*offset);
    }
